...
def serverSetup():
    """
    Creates a non-blocking daemon listening on ipv6 localhost port 9001 and sends lottery tickets back to client
    :return: theSocket, socketObject
   """
    host = '::1'
    port = 9001
    queueSize = 5

    theSocket = s.socket(s.AF_INET6, s.SOCK_STREAM)
    theSocket.setsockopt(s.SOL_SOCKET, s.SO_REUSEADDR, 1)  # prevents hijacking
    theSocket.bind((host, port, 0, 0))  # binds to localhost and port 9001
    theSocket.listen(queueSize)

    while True:
        try:
            socketObject, clientAddress = theSocket.accept()
            ...
            if errorCode == errno.EINTR:
                ...
                logzero.logger.error(f"Connection Failed {e.errno}")
#    return theSocket, socketObject
...
#        theSocket, socketObject = serverSetup()  # start connection and save socket
